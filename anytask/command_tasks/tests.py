...
from command_tasks.models import CommandTask, CommandTaskGroupRelations
...
class CreateTest(TestCase):
    def test_task_create_filled(self):
        ...
        parent_task = CommandTask.objects.create(title='parent_task',
                                          course=course)
        ...
        task.type = CommandTask.TYPE_SIMPLE
        ...
        task = CommandTask.objects.get(id=task_id)
        ...
        self.assertEqual(task.type, CommandTask.TYPE_SIMPLE)
...
class ViewsTest(TestCase):
    def setUp(self):
        ...
        self.task = CommandTask.objects.create(title='task_title_0',
                                        course=self.course,
                                        score_max=15)
    ...
    def test_task_create_page_anonymously(self):
        ...
        # get page
        response = client.get(reverse(command_tasks.views.task_create_page, kwargs={'course_id': self.course.id}))
    ...
    def test_task_import_page_anonymously(self):
        ...
        # get page
        response = client.get(reverse(command_tasks.views.task_import_page, kwargs={'course_id': self.course.id}))
    ...
    def test_task_edit_page_anonymously(self):
        ...
        # get page
        response = client.get(reverse(command_tasks.views.task_edit_page, kwargs={'task_id': self.task.id}))
    ...
    def test_task_create_or_edit_page_with_teacher(self):
        ...
        response = client.post(reverse(command_tasks.views.task_create_page, kwargs={'course_id': self.course.id}),
        ...
        # check created task
        self.assertEqual(len(CommandTask.objects.all()), 2, 'Must be 2 tasks')
        created_task = CommandTask.objects.get(id=2)
        ...
        self.assertEqual(created_task.type, CommandTask.TYPE_FULL, 'Created task wrong type')
        ...
        task_pos = CommandTaskGroupRelations.objects.filter(task=created_task)
        self.assertEqual(len(task_pos), 1, 'Created task CommandTaskGroupRelations len not 1')
        self.assertEqual(task_pos[0].group, self.group, 'Created task CommandTaskGroupRelations groups not equal')
        self.assertEqual(task_pos[0].position, 1, 'Created task CommandTaskGroupRelations position not 1')
        self.assertFalse(task_pos[0].deleted, 'Created task CommandTaskGroupRelations deleted')

        # get edit task page
        response = client.get(reverse(command_tasks.views.task_edit_page, kwargs={'task_id': created_task.id}))
    ...
    @patch('command_tasks.views.get_contest_info')
    def test_import_contest(self, mock_get_contest_info):
        ...
        problems = [{'problemId': '1055/2013_02_24/NuAYb8aSXw',
                     'problemTitle': 'CommandTask1',
                     'statement': 'text1',
                     'alias': 'A'},
                    {'problemId': '1055/2013_02_24/dY74Gd2fyz',
                     'problemTitle': 'CommandTask2',
                     'statement': 'text2',
                     'alias': 'B'},
                    {'problemId': '1055/2013_02_24/rqW5cRAAgR',
                     'problemTitle': 'CommandTask3',
                     'statement': 'text3',
                     'alias': 'C'}]
        ...
        response = client.post(reverse(command_tasks.views.contest_task_import), post_data)
        ...
        tasks = CommandTask.objects.exclude(id=1)
        ...
        for idx, task in enumerate(tasks):
            ...
            self.assertEqual(task.contest_integrated, True)
            self.assertEqual(task.rb_integrated, True)
            self.assertEqual(task.type, CommandTask.TYPE_FULL)
            ...
            self.assertEqual(task.sended_notify, False)
            task_pos = CommandTaskGroupRelations.objects.filter(task=task)
            self.assertEqual(len(task_pos), 1, 'Created task CommandTaskGroupRelations len not 1')
            self.assertEqual(task_pos[0].group, self.group, 'Created task CommandTaskGroupRelations groups not equal')

...
class CourseStatistics(object):
    ...
    def get_course_statistics(self):
        (self.course_statistics['middle_score'], self.course_statistics['middle_score_on_students'], self.course_statistics['ratio']) = self._calculate_scores_statistics(self.course_statistics['number_students'], 
    ...
    def add_statistics_for_group(self, group):
        ...
        for student in group.students.filter(is_active=True).order_by('last_name', 'first_name'):
            task_takens = TaskTaken.objects.filter(user = student).filter(task__in=self.course_tasks).filter(Q( Q(status=TaskTaken.STATUS_TAKEN) | Q(status=TaskTaken.STATUS_SCORED)))

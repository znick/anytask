...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Adding field 'Lesson.updated_by'
        db.add_column('lessons_lesson', 'updated_by',
                      self.gf('django.db.models.fields.related.ForeignKey')(blank=True, related_name='authors', null=True, db_index=False, to=orm['auth.User']),
                      keep_default=False)

        # Removing M2M table for field students on 'Lesson'
        db.delete_table(db.shorten_name('lessons_lesson_students'))
    ...
    def backwards(self, orm):
        # Deleting field 'Lesson.updated_by'
        db.delete_column('lessons_lesson', 'updated_by_id')

        # Adding M2M table for field students on 'Lesson'
        m2m_table_name = db.shorten_name('lessons_lesson_students')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('lesson', models.ForeignKey(orm['lessons.lesson'], null=False)),
            ('user', models.ForeignKey(orm['auth.user'], null=False))

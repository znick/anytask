...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        ...
        m2m_table_name = db.shorten_name('tasks_task_groups')
        ...
        # Adding M2M table for field groups on 'TaskLog'
        m2m_table_name = db.shorten_name('tasks_tasklog_groups')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('tasklog', models.ForeignKey(orm['tasks.tasklog'], null=False)),
            ('group', models.ForeignKey(orm['groups.group'], null=False))
        ))
        db.create_unique(m2m_table_name, ['tasklog_id', 'group_id'])


    def backwards(self, orm):
        # Removing M2M table for field groups on 'Task'
        db.delete_table(db.shorten_name('tasks_task_groups'))

        # Removing M2M table for field groups on 'TaskLog'
        db.delete_table(db.shorten_name('tasks_tasklog_groups'))
